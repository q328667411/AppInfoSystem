	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				...
				var c2=$("#categoryLevel2Id").val();
				if(c2!=""){
					$("#queryCategoryLevel2").val(c2).change();
				}
			},
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				...
				$("#queryCategoryLevel3").html(options);
				var c3=$("#categoryLevel3Id").val();
				if(c3!=""){
					$("#queryCategoryLevel3").val(c3);
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
				alert("加载三级分类失败！");
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="add?id="+obj.attr("appinfoid");
});
		...
		success:function(data){
			...
			if(data.errorCode === '0'){
				if(data.resultMsg === "success"){//操作成功
					...
					if("open" === obj.attr("saleSwitch")){
						alert("Date【"+obj.attr("appsoftwarename")+"】的【上架】操作失败");
					}else if("close" === obj.attr("saleSwitch")){
